...
Based on the binary detection accuracies, the ResNet-34 detector performance degrades to near random guessing against both PGD (50.30%) and BIM (50.05%) samples. Since both accuracies hover exactly around the 50% baseline for binary classification, it suggests that both iterative iterative attack methods are highly effective at evading detection. Neither attack is noticeably "easier" to detect than the other; the perturbations generated by both PGD and BIM are equally successful at fooling the defensive classifier into treating them as indistinguishable from clean images.
